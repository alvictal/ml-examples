...
'''
#Funtiction using Backward Elimination with p-values and Adjusted R Squared - need to study this yet
...
#Just a method to use a function to calculate backwardElimination automaticly
def backwardElimination(x, sl):
    numVars = len(x[0])
    for i in range(0, numVars):
        regressor_OLS = sm.OLS(y, x).fit()
...
#Non automatic way for backward elimination
'''
X_opt = X[:,[0,1,2,3,4,5]]
